...
def add_transition(ktn, origin, end, weight=0.0, origin_index=False, end_index=False):
    ...
    if origin_index:
        if not microstate_in(ktn, index=origin):
...
def microstate_in(ktn, name=None, index=None):
    ...
    return api_microstates.microstate_in(ktn.microstates, name=name, index=index)
...
def transition_in(ktn, origin, end, origin_index=False, end_index=False):
    ...
    return api_transitions.transition_in(ktn.transitions, origin, end, origin_index=True,
            end_index=True)
